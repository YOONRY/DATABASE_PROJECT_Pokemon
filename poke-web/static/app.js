document.addEventListener("DOMContentLoaded", () => {
    ...
    locations.forEach(loc => {
      ...
      li.addEventListener("click", () => {
        ...
        if (activeLocationId === loc.Lid) {
          ...
          document.getElementById("resultTitle").textContent = "지역을 선택하세요";
  ...
  async function loadEncounters(lid, lname) {
    ...
    rows
      .sort((a, b) => (a.Pid ?? 9999) - (b.Pid ?? 9999))
      .forEach(r => {
        const li = document.createElement("li");
        li.classList.add("pokemon-row");

        li.innerHTML = `
          <img class="sprite" src="/static/images/pokemon/${r.Pid}.png" alt="${r.Pname}">
          <div class="info">
            <div class="name">
  ...
  searchInput?.addEventListener("input", async () => {
    ...
    const res = await fetch(`/api/pokemon/autocomplete?q=${encodeURIComponent(keyword)}`);
    ...
    pokemons.forEach(p => {
      const li = document.createElement("li");
      li.innerHTML = `
        <img class="sprite" src="/static/images/pokemon/${p.Pid}.png" alt="${p.Pname}">
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target === searchInput || searchResult.contains(e.target)) return;
    searchResult.innerHTML = "";
  });
  ...
  async function loadLocationsByPokemon(pid, name) {
    ...
    document.getElementById("pokemonLocationTitle").textContent = `${name} 등장 지역`;
  ...
  async function loadPokemonsByType(type) {
    ...
    document.getElementById("typeTitle").textContent = `타입: ${type}`;
  ...
  async function loadEvolution(pid, name) {
    ...
    document.getElementById("evoTitle").textContent = `${name} 진화 트리`;
    ...
    evos.forEach(e => {
      ...
      li.innerHTML = `
        <div class="evo-pokemon">
          <img src="/static/images/pokemon/${e.from_pid}.png" alt="${e.from_name}">
          <div>${e.from_name}</div>
        </div>
        ...
        <div class="evo-pokemon">
          <img src="/static/images/pokemon/${e.to_pid}.png" alt="${e.to_name}">
          <div>${e.to_name}</div>
        </div>
      `;
